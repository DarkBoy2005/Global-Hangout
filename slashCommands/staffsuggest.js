...
module.exports = {
  name: "suggest2",
  description: "Submit a suggestion, used by server staffs/high commands to make a suggestion between them",
  cooldown: 0,
  memberpermissions: [],
  requiredroles: [],
  alloweduserids: [],
  ...
  run: async (client, interaction) => {
    ...
    try{
    	...
    	if(interaction.channel.id === staffchannel) {
	    	let msg = await client.channels.cache.get(staffsuggestions).send({content: '<@' + member.user.id + '>',embeds: [new MessageEmbed()
	    		.setTitle("Suggestion")
	    		.setColor("#ed8309")
	    		.setAuthor({
	        ...
	        embeds: [new MessageEmbed()
    			.setTitle(`${error2emoji} | Usage error`)
    			.setDescription(`Please use that in <#${staffchannel}>.`)
    			.setColor(ee.wrongcolor)],
	        ephemeral: true
    		})
    	}
