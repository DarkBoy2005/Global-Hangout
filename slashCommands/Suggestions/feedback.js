...
module.exports = {
  name: "feedback",
  description: "Give a feedback on a suggestion",
  cooldown: 1,
  memberpermissions: [],
  requiredroles: [],
  alloweduserids: [],
	    		...
	    		let uid = db.get(`${interaction.guild.id}.${id}.uid`)
			   		...
			   		interaction.channel.messages.fetch(`${msg2}`).then(msg3 => {msg3.reply({
					  	...
					  	.setTitle("Suggestion Feedback")
				   		.setColor("#ed8309")
				   		.setAuthor({
				   			name: `${taga}'s suggestion got a feedback`,
				        iconURL: `${avatara}`,
				        url: `https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${id}`
				   		...
				   		.setDescription(`**Feedback:** ${feedback}`)
							.setFooter({
								text: `Feedback from ${member.user.tag}`,
						    	...
						    	.addFields(
						    		{
										...
										{
											name: 'Status:', value: `Feedback received ${msgemoji}`, inline: true
										},
										{
				   	...
				   	try {
				   		...
				   		m.send({embeds: [new MessageEmbed()
				   			...
				   			.setDescription(`Your [suggestion](https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${id}) got a feedback in **${interaction.guild.name}**.`)
				   			.addFields(
				   				{
										name: 'Feedback:', value: `${feedback}`, inline: false
									},
