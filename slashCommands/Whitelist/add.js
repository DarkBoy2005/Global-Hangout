...
module.exports = {
  name: "add",
  description: "Add server to whitelist",
  cooldown: 0,
  memberpermissions: [],
  requiredroles: [],
  ...
  run: async (client, interaction) => {
    ...
    try{
      ...
      if(db.get(`${id}.set`) !== true){
        db.set(`${id}`, {set: true})
        return interaction.reply({content: `Server with ID \`${id}\` have been added to the whitelist`})
      } else {
        return interaction.reply({content: `This server is already in the whitelist.`})
      }
    } catch (e) {
