...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    try{
      ...
      if(db.get(`main.set`) === undefined){
        db.set(`main`, {set: id})
        return interaction.reply({content: `Server with ID \`${id}\` have been set as main`})
      } else {
        db.set(`main`, {set: id})
        return interaction.reply({content: `The main server ID have been updated to \`${id}\`.`})
      }
    } catch (e) {
